...
Pass `detection` to `createInput` to seed its escape-sequence trie with terminal-specific `key_*` sequences. Pass it to `createTerm` to gate color encoding (truecolor / 256 / 16-color SGR) and synchronized-output wrapping from the start.
